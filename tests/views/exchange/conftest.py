"""Fixtures compartidas para los tests de views/exchange/.

Replica del conftest de `tests/views/collections/`: AppContext seedeado
contra `:memory:` con un catálogo mínimo. Tests de exchange necesitan
un escenario con missing y duplicates para los flows del proposal.
"""
...
def ctx_with_demo() -> Iterator[AppContext]:
    ...
    Inventario inicial: ARG-1 qty=3 (1 owned + 2 dups), ARG-2 qty=0 (faltante),
    BRA-1 qty=1 (owned, no dup), BRA-2 qty=0 (faltante).
    """
    ...
    try:
        ...
        # Seed inventario: ARG-1 con duplicados, BRA-1 sin duplicados.
        ctx.inventory.add_card(coll.collection_id, "ARG", 1, 3)
        ctx.inventory.add_card(coll.collection_id, "BRA", 1, 1)
        ctx.conn.commit()
        yield ctx
    finally:
        ctx.close()
