...
- `requires_code=False`: solo se muestran Número y Cantidad. Foco inicial
  en Número. La búsqueda se hace con `InventoryService.find_cards_by_number`:
...
Vista preservada de v0.1 con sólo los callsites adaptados a la API de
servicios v0.2 (CLAUDE.md sec 7.1). La lógica de UI — validaciones,
máquina de ambigüedad, signals/slots, layout, navegación por Enter,
tinte Alta/Baja, manejo del completer — no se modifica.
"""
...
from collections_app.views.shared.widgets.enter_navigator import EnterNavigator
...
class CardLoaderView(QWidget):
    ...
    def _populate_completer_with_all_codes(self) -> None:
        ...
        lines = self._service.list_codes_for_collection(self.collection)
    ...
    def _validate_with_code(self, number: int) -> None:
        ...
        card = self._service.lookup_card(cid, code, number)
        if card is None:
    ...
    def _validate_by_number(self, number: int) -> None:
        ...
        matches = self._service.find_cards_by_number(cid, number)
        if not matches:
            self._clear_info()
            self._set_status(
    ...
    def _lookup_code_name(self, code_id: str) -> str:
        return self._service.lookup_code_name(self.collection.code_header_id, code_id)
    ...
    def _save_card(self) -> None:
        ...
        try:
            ...
            # Adaptación de v0.1: la vista atrapaba ValueError porque
            # InventoryService legacy lo usaba para todo. v0.2 usa
            # InventoryError; mismo tratamiento (status rojo + return).
    ...
    def _dispatch_save(
        ...
        if user_specified_code:
            ...
            if is_alta:
                return self._service.add_card(cid, code, number, qty)
            return self._service.remove_card(cid, code, number, qty)
        if is_alta:
            return self._service.add_card_by_number(cid, number, qty)
        return self._service.remove_card_by_number(cid, number, qty)
